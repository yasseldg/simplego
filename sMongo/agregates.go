...
func (c *Collection) AgregatesWithCtx(ctx context.Context, pipeline mongo.Pipeline, docs interface{}) error {
	...
	if err != nil {
		sLog.Error("sMongo: %sAgregatesWithCtx: %s", c.prefix(), err.Error())
	} else {
		err = cursor.All(ctx, docs)
		if err != nil {
			sLog.Error("sMongo: %sAgregatesWithCtx: cursor.All(): %s", c.prefix(), err.Error())
		}
	}
	return err
}
...
func (c *Collection) AgregatesCountWithCtx(ctx context.Context, pipeline mongo.Pipeline) ([]bson.M, error) {
	...
	if err != nil {
		sLog.Error("sMongo: %sAgregatesCountWithCtx: %s", c.prefix(), err.Error())
	} else {
		err = cursor.All(mgm.Ctx(), &result)
		if err != nil {
			sLog.Error("sMongo: %sAgregatesCountWithCtx: cursor.All(): %s", c.prefix(), err.Error())
		}
	}
	return result, err
}
